...
async def resolve_funcptr_target_via_mcp(
    ...
    *,
    ...
    code = (
        "import ida_bytes, ida_funcs, ida_segment, ida_ua, idautils, idc, json\n"
        "insn_ea = {insn_va}\n"
        "xref_matches = []\n"
        "for target_ea in idautils.DataRefsFrom(insn_ea):\n"
        "    func = ida_funcs.get_func(target_ea)\n"
        "    if func is not None and int(func.start_ea) == int(target_ea):\n"
        "        xref_matches.append(int(target_ea))\n"
        "operand_matches = []\n"
        "insn = ida_ua.insn_t()\n"
        "if ida_ua.decode_insn(insn, insn_ea) > 0:\n"
        "    direct_operand_types = {ida_ua.o_mem, ida_ua.o_near, ida_ua.o_far}\n"
        "    for op_index, operand in enumerate(insn.ops):\n"
        "        if operand.type == ida_ua.o_void:\n"
        "            break\n"
        "        if operand.type not in direct_operand_types:\n"
        "            continue\n"
        "        target_ea = int(idc.get_operand_value(insn_ea, op_index))\n"
        "        if target_ea == idc.BADADDR:\n"
        "            continue\n"
        "        if ida_segment.getseg(target_ea) is None:\n"
        "            continue\n"
        ...
        "            continue\n"
        "        func = ida_funcs.get_func(target_ea)\n"
        "        if func is not None and int(func.start_ea) == target_ea:\n"
        "            operand_matches.append(target_ea)\n"
        "matches = sorted(set(xref_matches + operand_matches))\n"
        "result = json.dumps({\n"
        "    'matches': [hex(target_ea) for target_ea in matches],\n"
        "    'xref_matches': [hex(target_ea) for target_ea in sorted(set(xref_matches))],\n"
        "    'operand_matches': [hex(target_ea) for target_ea in sorted(set(operand_matches))],\n"
        "})\n"
    )
    return await _resolve_unique_reference(session, insn_va, code, debug=debug)
