...
The reference function may decode `_HANDLE_TABLE_ENTRY->ObjectPointerBits` inline or through a named `ExGetHandlePointer` helper, while the matching target function may call an unnamed helper at the same semantic point. When the surrounding control flow and handle-entry operations match the reference, treat that unnamed helper call as the target's object-pointer decoding operation and emit `_HANDLE_TABLE_ENTRY->ObjectPointerBits` using the reference field layout. Do not return an empty result merely because the target helper has an IDA-generated `sub_` name or its body is not included in the target block.
...
- `found_struct_offset`: access to a requested regular struct field, including fields accessed through the semantically matching object-pointer decoding helper described above.
