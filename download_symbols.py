...
Usage:
    uv run python download_symbols.py [-xml=kphdyn.xml] [-symboldir=symbols] [-arch=amd64] [-version=10.0.19041] [-symbol_server=URL]
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help=f"Path to the XML file (default: {DEFAULT_XML_PATH}). Can be overridden by KPHTOOLS_XML environment variable."
    )
    parser.add_argument(
        "-symboldir",
        default=DEFAULT_SYMBOL_DIR,
        help=f"Directory to save downloaded symbols (default: {DEFAULT_SYMBOL_DIR}). Can be overridden by KPHTOOLS_SYMBOLDIR environment variable."
    )
    parser.add_argument(
        "-arch",
        default=None,
