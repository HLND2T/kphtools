    ...
    def test_returns_cli_specific_model_args(self) -> None:
        ...
        for agent_kind, model_args in expected_args.items():
            with self.subTest(agent_kind=agent_kind):
                self.assertEqual(
                    model_args,
                    agent_runner._agent_model_args(agent_kind, model_args[1]),
                )
...
class TestAgentCommandConstructionParity(unittest.TestCase):
    def setUp(self) -> None:
        agent_runner._MCP_PREFLIGHT_DONE.clear()
        agent_runner._MCP_PREFLIGHT_FAILED.clear()

    def test_claude_retry_reuses_session_with_absolute_cmd_executable(self) -> None:
        executable = r"C:\tools\claude.cmd"
        ...
        self.assertEqual(executable, first.args[0])
        self.assertIn(
            ["--model", "sonnet"],
            [first.args[index : index + 2] for index in range(len(first.args) - 1)],
        )
        self.assertIn(
        ...
        self.assertIn(
            ["--resume", "session-id"],
            [retry.args[index : index + 2] for index in range(len(retry.args) - 1)],
        )
        self.assertNotIn("--session-id", retry.args)
        self.assertIsNone(first.input_text)
        self.assertIsNone(retry.input_text)

    def test_codex_retry_uses_profile_model_stdin_and_latest_session(self) -> None:
        executable = r"C:\tools\codex.cmd"
        developer_instructions = 'developer_instructions="sig finder prompt"'
        ...
        self.assertEqual([executable, "--profile", "skill_runner"], first.args[:3])
        self.assertIn(
            ["-m", "gpt-5.4"],
            [first.args[index : index + 2] for index in range(len(first.args) - 1)],
        )
        self.assertEqual(["exec", "-"], first.args[-2:])
        self.assertEqual(["exec", "resume", "--last", "-"], retry.args[-4:])
        expected_prompt = "Run SKILL: .claude/skills/find-test/SKILL.md"
        self.assertEqual(expected_prompt, first.input_text)
        self.assertEqual(expected_prompt, retry.input_text)
    ...
    def test_codex_developer_instructions_strip_frontmatter(self) -> None:
        with TemporaryDirectory() as temp_dir:
            prompt_path = Path(temp_dir) / "sig-finder.md"
            prompt_path.write_text(
                "---\ndescription: test\n---\n\nUse the current IDA database.",
                encoding="utf-8",
            )

            result = agent_runner._load_codex_developer_instructions(prompt_path)

        self.assertEqual(
            'developer_instructions="Use the current IDA database."',
            result,
        )

    def test_codex_developer_instructions_reject_missing_and_empty_prompt(self) -> None:
        with TemporaryDirectory() as temp_dir:
            missing_path = Path(temp_dir) / "missing.md"
            empty_path = Path(temp_dir) / "empty.md"
            empty_path.write_text("---\ndescription: test\n---\n", encoding="utf-8")

            self.assertIsNone(agent_runner._load_codex_developer_instructions(missing_path))
            self.assertIsNone(agent_runner._load_codex_developer_instructions(empty_path))

    def test_display_command_redacts_developer_instructions(self) -> None:
        raw_args = [
            "codex",
            "-c",
            'developer_instructions="private prompt"',
            "exec",
            "-",
        ]

        display_args = agent_runner._display_command(raw_args)

        self.assertNotIn("private prompt", " ".join(display_args))
        self.assertIn("private prompt", " ".join(raw_args))

    @patch("agent_runner._ensure_agent_mcp_preflight")
    def test_unknown_agent_is_rejected_before_preflight(self, mock_preflight) -> None:
        self.assertFalse(agent_runner.run_skill("find-test", agent="custom-agent"))
        mock_preflight.assert_not_called()

    @patch("agent_runner.os.path.exists", return_value=True)
    @patch("agent_runner._ensure_agent_mcp_preflight", return_value=True)
    @patch("agent_runner._run_process_with_stream_capture", side_effect=FileNotFoundError)
    def test_missing_agent_during_skill_reports_failure_without_retry(
        self,
        mock_run_process,
        _mock_preflight,
        _mock_exists,
...
class TestProcessStreamCapture(unittest.TestCase):
    @patch("agent_runner.subprocess.Popen")
    def test_stdin_is_written_and_closed_while_streams_are_captured(self, mock_popen) -> None:
        process = _FakePopen(
        ...
        result = agent_runner._run_process_with_stream_capture(
            ["codex", "exec", "-"],
            agent_input="prompt",
        )

        self.assertEqual("prompt", "".join(process.stdin.writes))
...
class TestRunSkillOutputDetection(unittest.TestCase):
    ...
    @patch("agent_runner.os.path.exists", return_value=True)
    @patch("agent_runner._run_process_with_stream_capture")
    def test_run_skill_uses_one_attempt_when_retry_limit_is_zero(
        self,
        mock_run_process,
        _mock_exists,
    ) -> None:
...
class TestRunSkillCodexPromptTransport(unittest.TestCase):
    ...
    @patch.object(Path, "read_text", return_value="sig finder prompt")
    @patch(
        "agent_runner.os.path.exists",
        side_effect=lambda path: not str(path).endswith("missing.yaml"),
    )
    @patch("agent_runner._run_process_with_stream_capture")
...
class TestRunSkillMcpListPreflight(unittest.TestCase):
    ...
    def test_missing_ida_pro_mcp_blocks_agent_start(
        ...
        preflight_process = _FakePopen(
            stdout_chunks=["basic-memory: stdio - Connected\n"],
            stderr_chunks=[],
            returncode=0,
        )
