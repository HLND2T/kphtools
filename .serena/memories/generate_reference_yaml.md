...
- `generate_reference_yaml.py` - `parse_args`, `build_reference_output_path`, `infer_context_from_binary_path`, `resolve_func_va`, `resolve_reference_target`, `export_code_region_yaml_via_mcp`, `survey_current_binary_path`, `attach_existing_mcp_session`, `autostart_mcp_session`, `run_reference_generation`, `main`
...
## Architecture
The script is a thin CLI and session-lifecycle layer. `main()` parses arguments and runs the async workflow. `run_reference_generation()` chooses either attach mode or auto-start mode, obtains the binary hint, infers repository context from the binary path and `config.yaml`, resolves a `ReferenceTarget`, computes the reference YAML target path, and delegates to either function export or code-region export. Function targets export decompiler pseudocode and disassembly; `category: code` targets export only the configured address range's disassembly and do not include `procedure`.
...
flowchart TD
    ...
    I["resolve_reference_target(session, binary_dir, func_name)"]
    J["build_reference_output_path(repo_root, module, func_name, arch, outyaml)"]
    K{"target.kind == code?"}
    L["ida_reference_export.export_reference_yaml_via_mcp(...)"]
    M["ida_reference_export.export_code_region_yaml_via_mcp(...)"]
    N["ida_preprocessor_scripts/references/<module>/<name>.yaml"]
    A --> B
    B --> C
    C -->|"yes"| D
    C -->|"no"| E
...
Reference target resolution is staged. `resolve_reference_target()` first reads `<binary_dir>/<func_name>.yaml` through `symbol_artifacts.load_artifact`. For function artifacts, `func_va` is used directly and `func_rva` is converted with the IDA image base; if no artifact is present, `_lookup_function_start_addresses_by_exact_name()` asks IDA for an exact named address and requires exactly one unique function start. For `category: code` artifacts, `code_va` is used directly or `code_rva` is converted with the IDA image base, and `code_size` defines the half-open region `[code_va, code_va + code_size)`.

`ida_reference_export.export_reference_yaml_via_mcp()` owns the function export contract. It sends generated Python to IDA `py_eval`, writes YAML remotely, validates the remote acknowledgement, reads the written YAML back locally, and validates required fields. `ida_reference_export.export_code_region_yaml_via_mcp()` uses `ida_code_region_export_template.CODE_REGION_EXPORT_TEMPLATE` to render only the configured code range's `disasm_code`; code-region YAML intentionally omits `procedure`, while validation normalizes missing `procedure` to an empty string for shared consumers.
...
- Supported architectures are hard-coded to `amd64` and `arm64`; output path construction rejects any other architecture and rejects module/function/output names containing path separators or invalid filename characters.
...
- The MCP adapter automatically detects legacy worker versus supervisor contracts. With multiple active supervisor databases, attach mode requires `-mcp_database=<session_id>` and fails closed instead of guessing. Auto-start reference generation does not use the analyzer recovery budget: a matching supervisor IDB that is inactive or unreachable fails immediately as `ReferenceGenerationError`.
...
- `run_reference_generation()` calls `infer_context_from_binary_path()`, `resolve_reference_target()`, `build_reference_output_path()`, and then either `export_reference_yaml_via_mcp()` or `export_code_region_yaml_via_mcp()`.
