...
`ida_mcp_keepalive.keepalive_worker_during()` provides a 240-second worker keepalive around long remote waits. `ida_mcp_resolver._load_or_call_llm_result()` uses it only for the actual LLM await. Agent fallback also uses it when the lazy session already has a bound worker and injects that worker's dynamic MCP URL; the synchronous Agent CLI runs through `asyncio.to_thread()` so the owning event loop remains available for heartbeats. MCP protocol `ping` is handled by the supervisor and does not reach the worker, so the keepalive calls `py_eval("1")` through the already bound database session to refresh the worker's 600-second idle TTL. The keepalive task is cancelled and awaited after success or failure; a keepalive failure is logged only in debug mode and does not replace the foreground result or trigger background recovery.
